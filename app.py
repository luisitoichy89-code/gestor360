...
import sqlite3, secrets, os, datetime, math, hashlib, threading, time, shutil, uuid
...
.footer,.footer-login{margin-top:auto}
...
def page_full(title, body, include_confirm=True, rol='seller', nombre='', almacen='1'):
    ...
    return ("<!DOCTYPE html><html lang='es'><head>"
            ...
            "<body>" + sb + body + c + FOOTER_HTML + "</body></html>")
...
def pin_verification():
    ...
    error = ''
    if request.method == 'POST':
        ip = request.remote_addr; pin = request.form['pin'].strip()
        att = PIN_ATTEMPTS.get(ip,0)
        if att >= 5: error='<div class="error">⛔ Demasiados intentos</div>'
        elif pin == u.get('pin',''):
...
def locales():
    ...
    locs = db.execute("SELECT * FROM locales WHERE nombre LIKE ? OR CAST(id AS TEXT) LIKE ? ORDER BY id",
                      ('%'+q+'%','%'+q+'%')).fetchall() if q else db.execute("SELECT * FROM locales ORDER BY id").fetchall()
    ...
    for l in locs:
        ...
        h += ('<div class="lic-card">'
              ...
              '</div></div></div>')
    h += ('<h3>Nuevo Local</h3>'
          '<div class="form-group"><label>Nombre</label><input type="text" id="lNom" placeholder="Ej: Tienda Centro"></div>'
          '<div style="padding:0 14px"><button onclick="confirmar(\'¿Crear local?\',\'\',function(ok){if(ok)crearLocal()})">🏪 Crear Local</button></div>')
...
def api_crear_local():
    ...
    db.execute("INSERT INTO locales (nombre,activo,created_at) VALUES (?,1,datetime('now'))",(d['nombre'],))
...
def api_editar_local():
    if session.get('user',{}).get('rol')!='superadmin': return jsonify({'ok':False})
    d=request.json; db=get_db()
    db.execute("UPDATE locales SET nombre=?,activo=? WHERE id=?",(d['nombre'],int(d['activo']),d['id']))
    db.commit(); return jsonify({'ok':True})
...
def licencias():
    if session.get('user',{}).get('rol')!='superadmin': return redirect(url_for('dashboard'))
    db=get_db()
    lics=db.execute("SELECT l.*,lo.nombre as lnombre FROM licencias l LEFT JOIN locales lo ON lo.id=CAST(l.almacen_id AS INTEGER) ORDER BY l.id DESC").fetchall()
    locs=db.execute("SELECT * FROM locales WHERE activo=1 ORDER BY id").fetchall()
    total=len(lics)
    activas=sum(1 for l in lics if l['activo'] and (not l['expiracion'] or datetime.datetime.strptime(l['expiracion'],'%Y-%m-%d') >=datetime.datetime.now()))
    ...
    h+=('<div class="stat-row">'
         ...
         '</div><h3>Dispositivos registrados</h3>')
    for l in lics:
        exp=l['expiracion'] or '—'
...
def api_add_licencia():
    ...
    d=request.json; db=get_db()
    dias=d.get('dias')
    exp=(datetime.datetime.now()+datetime.timedelta(days=int(dias))).strftime('%Y-%m-%d') if dias and int(dias)>0 else ''
    db.execute("INSERT OR REPLACE INTO licencias (device_id,almacen_id,expiracion,activo,created_at) VALUES (?,?,?,1,datetime('now'))",
               (d['device_id'],d['almacen_id'],exp))
    db.commit(); return jsonify({'ok':True})
...
def usuarios():
    if session.get('user',{}).get('rol')!='superadmin': return redirect(url_for('dashboard'))
    db=get_db(); q=request.args.get('q','')
    users=db.execute("SELECT * FROM usuarios WHERE (username LIKE ? OR nombre LIKE ? OR almacen_id LIKE ?) AND rol!='superadmin' ORDER BY rol,username",('%'+q+'%','%'+q+'%','%'+q+'%')).fetchall() if q else db.execute("SELECT * FROM usuarios WHERE rol!='superadmin' ORDER BY rol,username").fetchall()
    locs=db.execute("SELECT * FROM locales WHERE activo=1 ORDER BY id").fetchall()
    ...
    if admins:
        h += '<h3>👤 Administradores / Jefes (' + str(len(admins)) + ')</h3>'
        for u in admins: h += render_u(u)
    if sellers:
        h += '<h3>🧑 Vendedores (' + str(len(sellers)) + ')</h3>'
...
def crear_usuario():
    ...
    try:
        db.execute(
            ...
            (d['username'], '', d.get('nombre', d['username']),
             d['rol'], d.get('pin',''), d['almacen_id']))
        db.commit()
...
def api_editar_usuario():
    if session.get('user',{}).get('rol')!='superadmin': return jsonify({'ok':False})
    d=request.json; db=get_db()
    upd="nombre=?,almacen_id=?,rol=?,activo=?"; params=[d.get('nombre',''),d.get('almacen_id','1'),d.get('rol','seller'),int(d.get('activo',1))]
    if d.get('password') and len(d['password'])>=6: upd+=",password=?"; params.append(hashlib.sha256(d['password'].encode()).hexdigest())
    if d.get('pin'): upd+=",pin=?"; params.append(d['pin'])
    params.append(d['id'])
...
<div id="msgs" style="height:56vh;overflow-y:auto;padding:12px;display:flex;flex-direction:column;gap:5px"></div>
<div style="display:flex;gap:8px;padding:10px 14px;background:rgba(0,0,0,0.18)">
<input type="text" id="cInp" placeholder="Escribe un mensaje..." style="flex:1;margin:0">
<button onclick="cSend()" style="width:auto;padding:11px 16px;margin:0">Enviar</button>
</div>
<script>
var KEY='chat_"""+a+"""',UN='"""+uname+"""';
function load(){var ms=JSON.parse(localStorage.getItem(KEY)||'[]'),el=document.getElementById('msgs');
el.innerHTML='';ms.slice(-50).forEach(function(m){var me=m.user===UN;
el.innerHTML+='<div style="display:flex;flex-direction:column;align-items:'+(me?'flex-end':'flex-start')+'">'
+'<div style="font-size:10px;color:#9FA8DA;margin-bottom:2px">'+(me?'':m.user+' — ')+m.time+'</div>'
+'<div class="chat-bubble '+(me?'me':'other')+'">'+m.text+'</div></div>'});el.scrollTop=el.scrollHeight}
...
@app.route('/seguridad')
def seguridad():
    return pol("Seguridad","<h2>Política de Seguridad</h2><p>Actualización: 14/06/2026</p><br><h3>1. Protección</h3><p>SHA256, rate limiting 5 intentos, backup cada hora.</p><br><h3>2. Roles</h3><p>Superadmin / Admin (PIN 5 min) / Vendedor.</p><br><h3>3. Contacto</h3><p>luisi26@nauta.cu</p>")
